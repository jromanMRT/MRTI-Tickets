...
router.get('/options', requireCoreAuth, async (_req, res) => {
  try {
    ...
    res.json({ success: true, data: { categories, priorities } });
  } catch (err) {
    console.error(err);
...
router.post('/', requireCoreAuth, async (req, res) => {
  ...
  try {
    if (categoryId) {
      const [categories]: any = await pool.query('SELECT id FROM ticket_categories WHERE id = ? AND active = 1 LIMIT 1', [categoryId]);
      if (!categories.length) return res.status(400).json({ success: false, error: { code: 'INVALID_CATEGORY', message: 'La categoría seleccionada no está disponible' } });
    }
